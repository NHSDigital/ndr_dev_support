...
module RakeCI
  module CommitCop
    ...
    class MissingAssociatedTestFileTest < Minitest::Test
      def setup
        NdrDevSupport::RakeCI::CommitCop.tested_paths = ['app/tested_path']
      ...
      def test_should_not_respond_to_commit_with_new_file_with_associated_test
        @changes[:added].add('app/tested_path/new_file.rb')
        @changes[:added].add('test/tested_path/new_file_test.rb')

        @changes[:added].add('app/tested_path/namespace/new_file.rb')
        @changes[:added].add('test/tested_path/namespace/new_file_test.rb')
        assert_nil @cop.check(@changes)
      end

      def test_should_respond_to_commit_with_new_file_but_test_is_not_associated
        @changes[:added].add('app/tested_path/users_controller.rb')
        @changes[:added].add('test/tested_path/incorrect_test.rb')
      ...
      def test_should_not_respond_to_untested_path
        @changes[:added].add('app/untested_path/new_file.rb')
        assert_nil @cop.check(@changes)
      end
    end
  end
